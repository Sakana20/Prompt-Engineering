...
提供新信息。表达顺序由当前商品、受众和传播目标决定，不使用固定开头或信息比例；仅当当前
`ValidationConfig` 配置了结尾行动引导语时，必须以其中一条完整收尾。
...
- 字符范围、明确禁词、行动引导禁用词、必需的结尾行动引导语、格式前缀和数字红包金额规则
  只取当前 `ValidationConfig`。配置了 `required_ending_call_to_actions` 时，正文必须以其中
  一条完整句式结束，句后不得再添加内容。
