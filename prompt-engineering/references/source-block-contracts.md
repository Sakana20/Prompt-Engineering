...
`volume-copy-source-blocks.md` 的“审核发布的网页学习块”一节接收通过 `learning-publish`
登记的新增块，并进入同一验证注册表。Codex 必须在发布清单中声明品类族、消费需求、季节、
适用模式、强类型插槽、已删除风险与多样性标签；这些结构化字段由本地发布审计保存，不写进
正式原文库。正式库仍只保存原有“块标题 + `text` 文案”格式。选块仍先判断品类和需求，不能
因为是网页学习块而绕过本文件规则。

确定性验证从正式块的方括号占位符直接计算必需填槽数，并依据块内明确的饮品词判断饮品兼容
性。因此 Codex 发布饮品块时必须使用“饮品名称”“已确认饮品内容”等可辨认的强类型插槽，
不能把饮品类型只藏在发布清单里。
