...
- `save-dreamina-video-node` 读取上述 CSV/interface，用真实 `image_node_id` 替换
  ...
  Prompt 内嵌文案，不得省略、改写、截断或提前结束；Dreamina 专用清洗会去掉不得
  手持、拿起或触碰商品、固定机位、禁止切镜和禁止运镜类限制，并鼓励人物根据口播语义自然
  接触、拿起或使用商品。当前用户指定视频模型为 `seedance_2.0mini`，执行前仍须通过实时
  `model find` 确认 canonical model。静态首帧及其他下游适配器不受影响。
