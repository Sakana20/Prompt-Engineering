...
- 在原审核台增加无刷新工作台切换、左栏日期扫描、中间主列表媒体多选与显式转写、学习候选
  编辑、右侧媒体预览、逐项失败原因与重试选择、409 冲突、状态动作、人物 Prompt 新增和静态
  资源防缓存；
- FunASR worker 使用隔离 Python 环境并在识别前预检依赖导入，避免继承 Prompt Engineering
  的 Python 环境；
- 新候选保留不可变 ASR 原文和时间轴，初始可编辑稿只执行确定性空白与重复标点清洗，不做
  语义纠错或标点推断；
- 审核分类字段使用服务端枚举和中文控件；草稿候选支持 revision 保护的可恢复删除，删除后
  素材恢复未识别并可重新生成，批准或发布候选禁止删除；
