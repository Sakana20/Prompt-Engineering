...
- 每条只选择一个完整真人原文块，只允许填槽和移除换行，不允许润色、仿写或拆块拼接；
- 同一批次不重复 `source_block_id`，降低批量重复度；
...
验收：模板直接包含真人原文块、只填槽约束、样本利益点隔离和强制校验指令；已知样本利益点
不会进入分发 Skill 或生成 Prompt，模型不得根据样本文风自行仿写。
