...
- `seedance_2.5` 的 `m2v` 模式允许 1–50 个素材引用，并允许只提供图片素材。
...
Dreamina 视频适配层会移除静态首帧带入的“不接触商品”、固定机位、不切镜、一镜到底、
不运镜和不推拉摇移等限制；静态首帧、Oceanengine 和 LibTV 规则不变。
...
- 视频默认候选：`seedance_2.5 + m2v + 9:16 + 720p`；
- 视频输入只有 `image`；
