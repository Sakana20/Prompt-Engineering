...
文案模板 `2026-07-02-natural-v4` 使用弱结构：每条只讲一个连续生活片段，让需求从
具体动作或物件自然产生；利益点嵌入人物动作或念头；商品信息缩减为 1–2 个已确认的
使用动作或可观察细节；结尾不再强制回扣抵扣、省心或方便。确定性校验仍负责字数、
固定利益点、禁词、行动引导和单段格式，口语自然度由 Codex 在生成与人工审核阶段判断。

生成态利益点使用 `[[NO_SPLIT]]…[[/NO_SPLIT]]` 标注，标签不计入口播字数。产物在此
扇出：每个任务写一份保留标签的 `<task_id>.smartsplit.txt` 字幕稿；每个批次写一份
Oceanengine CSV，`script` 去掉标签。两类 writer 独立调用，任何一方都不能隐式触发、
覆盖、导入或运行另一方。

Skill 使用透明 CLI 启动器调用仓库入口，不复制或删减底层参数。CLI 与 Skill 配置分别由
`cli-parameters.schema.json`、`skill-config.schema.json` 描述。批处理、插件声明、安全
调试以及 text、JSON、CSV、Markdown 输出约定在 `runtime.md` 中；未知参数由启动器原样
转发，避免封装层阻断底层未来能力。
