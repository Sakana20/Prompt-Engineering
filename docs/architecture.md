...
文案模板 `2026-08-05-temporal-source-blocks-v15` 定位为“商品导向的生活化分享”：场景或需求用
...
跑量样本不再经过风格提炼或模型仿写，而是由 `volume-copy-source-blocks.md` 保存审核后的
真人原句块。生成端每条选择一个完整块，只替换事实插槽并移除换行，不润色、不调整句序、
不拆块拼接；同批次不重复 `source_block_id`。样本中的金额、价格、券、红包、活动、平台
...
12–2 月冬的确定性规则拒绝跨季节表达。实时雨雪、刮风、升降温等天气描述默认不可信，只有
