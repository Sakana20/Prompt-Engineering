...
首帧，而不是生活方式抓拍；人物说话时必须直视镜头并保持稳定镜头交流。背景优先使用带
2–3 个协调可见元素的真实店内或温馨居家环境，禁止大白墙和纯白空墙。场景只作为背景，
...
候选创建把 ASR 全文和逐字时间轴保存为不可变证据；初始 `edited_transcript` 由本项目的
确定性清洗器生成，只移除中文字符间空白、压缩明显重复标点，并保留相邻 ASCII 字母数字词组
之间的单个空格。清洗器只保留 ASR 已有标点，不补写缺失标点、不改字、不改变 token 或
时间戳。人工在审核台补标点或纠错后，只产生新的候选 revision，不会反写不可变证据。
