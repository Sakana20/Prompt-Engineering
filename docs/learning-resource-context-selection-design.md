...
- 文案阶段已完成：固化块和网页发布块统一从正式 Markdown 解析，单条最多选择 4 块，批量按
  `max(4, floor(N/2)+2)` 选择，并受 7,000 字符硬上限约束；PromptPackage 记录实际候选 ID
  和字符数，不再从模板或发布区全文注入。跨季或品类不兼容块如进入限量候选，只能标为
  `human_rewrite` 参考，不得用于 `source_fill`。

## 推荐决策

阶段 A 与阶段 B 均已实施。正式学习资料仍完整保存，CLI/Codex只消费本次需要的紧凑候选集；
后续根据真实生成回归调整排序标签和预算，不改变正式资源格式。
